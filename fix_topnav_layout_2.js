...
const oldDropdownStart = content.indexOf('{showProfileMenu && user && (');
if (oldDropdownStart !== -1) {
  const replacement = `{showProfileMenu && user && (
   ...
   content = content.substring(0, oldDropdownStart) + replacement + content.substring(content.lastIndexOf('</nav>'));
   fs.writeFileSync(file, content, 'utf8');
   console.log('Fixed TopNav via direct replace');
}
